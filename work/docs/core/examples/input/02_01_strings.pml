[ch (id=string) [title Strings]
    ...
    Here we define a node with [link (url="^[ins-const core_spec_url]#tag_def") tag] [c string], containing the string [c This is some arbitrary string].
    The node starts with [c \[] (opening delimiter) and ends with  [c \]] (closing delimiter).
    The tag and content of the node are [link (url="^[ins-const core_spec_url]#node_separator") separated] by a space.
    ...
    You can use new lines and any other Unicode code points in strings, except control characters listed in [link (url="^[ins-const core_spec_url]#invalid_characters") Invalid Characters].
    ...
    [note
        ...
        PDML documents [i must] be [link (url=^[ins-const core_spec_url]#encoding) encoded in UTF-8].
        Unicode surrogate pairs are therefore never used in PDML — they are a mechanism specific to UTF-16 encoding.
    ]

    Some characters [i must] be escaped, and some [i can] be escaped (see [link (url="^[ins-const core_spec_url]#character_escape_sequences") Character Escape Sequences]).
